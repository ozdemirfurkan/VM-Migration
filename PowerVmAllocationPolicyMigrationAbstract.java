...
public abstract class PowerVmAllocationPolicyMigrationAbstract extends PowerVmAllocationPolicyAbstract {
	...
	public PowerHost findHostForVm(Vm vm, Set<? extends Host> excludedHosts) {
		/**
		 * The method parameter determines the 
		 * algorithm for finding host for VM.
		 * 
		 * method = 0 - > PABFD (default)
		 * method = 1 - > WFD (Worst Fit Decreasing)
		 * method = 2 - > SWFD (Second Worst Fit Decreasing)
		 * method = 3 - > FFD (First Fit Decreasing)
		 * method = 4 - > FFD with decreasing host
		 */
		...
		if(method == 0) {
			...
			double maxPower = Double.MIN_VALUE;
			PowerHost allocatedHost = null;
			Collections.sort(this.<PowerHost> getHostList(), new Comparator<PowerHost>() {
	            @Override
	            public int compare(PowerHost host1, PowerHost host2) {
	                return (int) (getUtilizationOfCpuMips(host2) - getUtilizationOfCpuMips(host1));
	            }
	        });
			for (PowerHost host : this.<PowerHost> getHostList()) {
				if (excludedHosts.contains(host)) {
